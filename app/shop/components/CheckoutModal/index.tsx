...
export function CheckoutModal({ 
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Genereer een orderNumber in formaat JJJJ/XX
      const year = new Date().getFullYear()
      // Normaal zou je deze uit een database halen, maar voor test gebruiken we timestamp
      const orderNum = `${year}/${Math.floor(Math.random() * 99).toString().padStart(2, '0')}`
      setOrderNumber(orderNum)
      
      // In plaats van een API call te maken, tonen we direct de betalingsinformatie
      // Maar je zou hier nog steeds de order in een database kunnen opslaan
      
      // Toon succes bericht
      setSuccess(true)
      
      // We laten de cart nog even bestaan zodat de gebruiker de factuurgegevens kan zien
  ...
  if (success) {
    ...
    return (
      <Modal isOpen={isOpen} onClose={() => {
        ...
        <div className="text-white max-h-[90vh] overflow-y-auto p-6 max-w-3xl mx-auto">
          ...
          <div className="flex justify-between gap-4">
            <button
              onClick={() => {
                // Download als PDF functionaliteit zou hier komen
                // Voor nu plaatshouder
